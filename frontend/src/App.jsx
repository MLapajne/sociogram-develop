      ...
      <Routes>
        ...
        {formData &&
          ...
          formData.sociograms.map((sociogram) => {
            //console.log("dataContext:", dataContext); // Print dataContext
            //console.log("publishedUsers:", sociogram.publishedUsers); // Print dataContext
            return sociogram.users
              .filter((usr) => {
                // Check if dataContext includes usr.id
                return !sociogram.publishedUsers.includes(usr.id);
              })
              .map((usr) => (
                <Route
                  key={usr.id}
