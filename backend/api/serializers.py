...
class SociogramSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Sociogram
        fields = ['id','instructorName', 'description', 'language', 'negQuestions', 'posQuestions' ,'users', 'publishedUsers']
...
class SubmitFrontendDataSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id','sociogramId', 'firstName' ,'lastName', 'createdAt', 'gender', 'users', 'posQuestions', 'negQuestions']
    ...
    def create(self, validated_data):
        ...
        # Pass user IDs to SociogramSerializer
        #sociogram_serializer = SociogramSerializer(data={'publishedUsers': [submitFrontendData.id]})
        #if sociogram_serializer.is_valid():
        #    sociogram_serializer.save()

        sociogram.publishedUsers.append(submitFrontendData.id)
        sociogram.save()
